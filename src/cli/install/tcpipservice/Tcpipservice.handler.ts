...
import { AbstractSession, ICommandHandler, IHandlerParameters, IProfile, ITaskWithStatus, TaskStage } from "@zowe/imperative";
import { ICMCIApiResponse, installTcpipservice } from "../../../api";
import { CicsBaseHandler } from "../../CicsBaseHandler";
...
/**
 * Command handler for installing CICS tcpipservice via CMCI
 * @export
...
export default class TcpipserviceHandler extends CicsBaseHandler {
    public async processWithSession(params: IHandlerParameters, session: AbstractSession, profile: IProfile): Promise<ICMCIApiResponse> {
        ...
        const status: ITaskWithStatus = {
            statusMessage: "Installing tcpipservice to CICS",
            percentComplete: 0,
            stageName: TaskStage.IN_PROGRESS
        };
        params.response.progress.startBar({task: status});

        const response = await installTcpipservice(session, {
