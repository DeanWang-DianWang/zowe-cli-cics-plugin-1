...
describe("CICS install pipleine command", () => {

    beforeAll(async () => {
        TEST_ENVIRONMENT = await TestEnvironment.setUp({
            testName: "install_pipeline",
            installPlugin: true,
    ...
    it("should be able to display the help", () => {
        const output = runCliScript(__dirname + "/__scripts__/install_pipeline_help.sh", TEST_ENVIRONMENT, []);
        expect(output.stderr.toString()).toEqual("");
        expect(output.status).toEqual(0);
        expect(output.stdout.toString()).toMatchSnapshot();
    });

    it("should get a syntax error if pipelineName is omitted", () => {
        const output = runCliScript(__dirname + "/__scripts__/install_pipeline.sh", TEST_ENVIRONMENT, ["", "FAKEGRP", "FAKEREG"]);
        const stderr = output.stderr.toString();
        expect(stderr).toContain("Syntax");
        expect(stderr).toContain("Missing Positional Argument");
        expect(stderr).toContain("pipelineName");
